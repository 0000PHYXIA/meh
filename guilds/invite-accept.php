...
<?php
	...
	if($row['GuildInvite']==0) {
		echo '<div id="fail">You can not accept a guild since no one invited you.</div>';
	} else {
		$guildinvid = $row['GuildInvite'];
		mysql_query("UPDATE meh_users SET guild='$guildinvid' WHERE Username='$username'");
		mysql_query("UPDATE meh_users SET GuildInvite='0' WHERE Username='$username'");
		echo '<div id="success">You have successfully joined the guild where you got invited.</div>';
	}
?>